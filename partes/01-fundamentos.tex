...
Antes de qualquer tiro ser disparado, é preciso entender a espinha dorsal que sustenta cada decisão em Vanguarda 44: como as ordens são dadas, quem manda em quem, e o que significa, na prática, ativar uma unidade em pleno caos de batalha. Esta parte estabelece as regras fundamentais: os alicerces sobre os quais tudo o mais no livro é construído.
...
\textbf{Convenção Universal de Dados}: em qualquer rolagem de 1D6 para Acertar ou executar um Teste no jogo (incluindo perfis especiais como Bazooka, Granadas, Tiro Selecionado do Sniper e Ajuste de Mira de Morteiro), um resultado natural de 6 sempre é um sucesso, ignorando qualquer modificador que exigiria um valor maior. Um resultado natural de 1 sempre é uma falha, mesmo que modificadores favoráveis reduzissem o alvo para menos de 1.
...
\begin{itemize}[leftmargin=1.2cm]
  ...
  \item \textbf{Reagrupar (Rally)}: tentar reduzir o Nível de Stress (veja Parte III, Moral e Stress).
  \item \textbf{Emboscada (Ambush)}: esperar que uma unidade inimiga se mova ou atire para então reagir.
  \item \textbf{Abaixar (Down)}: ficar prostrado para defesa máxima. Uma unidade em Abaixar concede -1 adicional no dado de quem atira nela, empilhando com a cobertura do terreno que ela já tiver (veja Parte III, Sequência de Disparo).
\end{itemize}

\begin{nota}
Ao receber a ordem Emboscada (mesmo custo de PC e Teste de Ordem das demais ordens), a unidade permanece parada (não move nem atira neste turno) e fica ``Em Emboscada'' até sua próxima ativação. Na sua próxima ativação, se a unidade decidir Atirar, ela recebe +1 no Teste de Acerto (representando a posição preparada e o elemento surpresa).
\end{nota}
...
\textbf{TN base}: 3+ para qualquer unidade, independentemente do seu Nível de Stress.

Mesmo esquadrões descansados e sem Stress algum enfrentam essa margem de incerteza. Ela representa a fricção inevitável da guerra: ordens mal-entendidas, rádios que falham, um mensageiro que demora a chegar. Nem toda falha em batalha vem do medo.
...
\begin{nota}
Os efeitos do Stress (movimento, precisão e alcance) são aplicados diretamente conforme o Nível do dado vermelho (veja Parte III, Moral e Stress) e não alteram este teste.
\end{nota}
...
\textbf{Vermelho Escuro (Vinho)}: identifica o Tenente (Oficial), que gera 2 PC por turno.

\textbf{Amarelo Ocre (Mostarda)}: identifica os Sargentos (NCOs) de esquadrão, que geram 1 PC por turno.

\textbf{Laranja}: identifica o Capitão, que gera 3 PC por turno.
