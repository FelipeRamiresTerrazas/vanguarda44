...
\hypersetup{
  pdftitle={Vanguarda 44 -- Guia Rapido de Inicio},
...
\begin{center}
{\bfseries\color{vinho}\Huge VANGUARDA 44\par}
\vspace{6pt}
...
O jogo gira em torno de duas ideias centrais. A primeira são os \textbf{Pontos de Comando (PC)}, que determinam quantas unidades você consegue ativar e mandar agir em cada turno. A segunda são os \textbf{Marcadores de Stress (Pins)}: o medo se acumula, e uma unidade sob fogo pesado fica pior de mirar e pode travar de medo, mesmo sem sofrer baixas.
...
Quando ativada, uma unidade escolhe uma entre cinco ordens: Avançar (move e atira com -1), Correr (move o dobro, não atira), Atirar (parada, força total), Reagrupar (não move nem atira, tenta remover Stress) ou Abaixar (defesa máxima, não move nem atira). Para a ordem funcionar, role 1D6 contra um Teste de Ordem, com TN 3+ base, mais 1 por Marcador de Stress na unidade. Se falhar, a unidade vai para Abaixar e não ativa de novo neste turno.

\begin{quote}
\itshape Exemplo: uma unidade com 2 Pins precisa de 5+ para executar sua ordem (3 base + 2 de penalidade).
\end{quote}
...
No \textbf{Passo C --- Consequências}, cada sucesso obtido no Passo B remove 1 miniatura do defensor. Além disso, se houve pelo menos 1 acerto no Passo A --- mesmo sem causar baixa --- a unidade alvo recebe 1 Marcador de Stress (armas como LMG e HMG causam 2).

\begin{quote}
\itshape Exemplo: 8 fuzileiros atiram contra um esquadrão regular em cobertura leve, sem se mover. Precisam de 5+; 3 acertam. Esses 3 dados rolam de novo, precisando de 4+; 1 causa baixa. O defensor remove 1 miniatura, e a unidade recebe 1 Marcador de Stress.
\end{quote}

\section{Moral e Marcadores de Stress (Pins)}

Cada Pin acumulado dá -1 no Acerto da unidade. Para ativar uma unidade que já tem Pins, role 2D6 e compare contra a Moral da unidade menos os Pins acumulados: se o resultado for igual ou menor, a ativação funciona normalmente. A Moral varia por qualidade de tropa, e é justamente esse valor que define quantos Pins uma unidade aguenta antes de entrar em pânico.
...
\toprule
\rowcolor{vinho}
\textcolor{white}{\textbf{Qualidade}} & \textcolor{white}{\textbf{Moral}} & \textcolor{white}{\textbf{Foge com}} \\
\midrule
Inexperiente & 8 & 8 Pins \\
Regular & 9 & 9 Pins \\
Veterana & 10 & 10 Pins \\
\bottomrule
\end{tabular}
...
Se os Pins de uma unidade igualarem sua Moral, ela entra em pânico e é removida do jogo.

Para remover Pins, existem duas opções: a própria unidade pode receber a ordem Reagrupar, que não permite mover nem atirar naquele turno mas rola 1D6 para determinar quantos Pins são removidos; ou, se o Sargento da unidade já morreu, um Tenente a até 15cm pode gastar 1 PC para ordenar Reagrupar nela.
...
A Moral também fica mais simples: toda unidade foge ao acumular 6 Pins, não importa sua qualidade de tropa. A partida tem limite de 6 turnos, e vence quem tiver cumprido mais objetivos --- ou, em caso de empate, quem tiver menos baixas.
