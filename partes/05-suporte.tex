...
Rifles e metralhadoras vencem guerras de atrito, mas são os especialistas (o atirador de precisão escondido, a equipe de morteiro invisível atrás de uma colina, o sargento que sabe exatamente quando dividir o fogo do seu esquadrão) que decidem batalhas. Esta parte cobre as armas e táticas avançadas que dão a Vanguarda 44 sua profundidade tática.

\section{Armas Especiais e Equipes de Suporte}

Além dos rifles e SMGs padrão, Vanguarda 44 utiliza armas especiais que podem estar integradas a um Esquadrão de Infantaria (LMG, Bazooka) ou operar como Unidades de Suporte independentes (HMG, Sniper). Todas compartilham um traço em comum: são operadas por equipes pequenas, onde a perda de um auxiliar enfraquece a arma, mas nunca a silencia.

Antes de detalhar cada arma individualmente, a tabela abaixo resume os alcances de referência usados no jogo, útil para consulta rápida durante a partida:
...
Arma & Alcance \\
\midrule
Submetralhadora (SMG) & 20cm \\
Rifle & 60cm \\
Metralhadora Leve (LMG/BAR) & 60--80cm \\
Granada & 15cm (só contra Cobertura Pesada) \\
Bazooka (contra infantaria) & Curto/Contato (perfil especial) \\
Sniper & 100cm (ignora penalidade de longa distância) \\
Morteiro Leve & 30cm -- 60cm \\
...
\begin{nota}
A MG42 alemã disparava a uma cadência tão alta (até 1200 tiros por minuto) que soldados aliados a apelidaram de ``serra elétrica de Hitler'' pelo som distintivo que fazia em combate.
\end{nota}
...
\begin{nota}
A Bazooka M1 entrou em combate pela primeira vez na Operação Tocha, no Norte da África, em 1942, e revelou rapidamente suas limitações contra a blindagem frontal dos tanques alemães mais pesados, levando a versões mais potentes ao longo da guerra.
\end{nota}

\subsection{Bazooka contra Infantaria (Poder de Fogo Explosivo)}

Além de sua função Anti-Tanque, a equipe de Bazooka/Panzerschreck pode direcionar seu disparo contra infantaria, usando um perfil simplificado de dano de área, que substitui a Sequência de Disparo padrão da Parte III: resolve-se com uma única rolagem de Acerto seguida de uma única rolagem de Dano:
...
\begin{nota}
Na prática, isso força uma escolha tática real toda vez que um Assistente cai: manter a arma atirando (pior, mas presente) ou sacrificar o turno de tiro para reposicionar a arma em segurança. As regras específicas de cada arma (Metralhadora Pesada, Equipe de Sniper, Morteiros) remetem a esta seção em vez de repetir o texto.
\end{nota}
...
\textbf{Poder de Negação}: sempre que a HMG acertar uma unidade inimiga, aquela unidade sobe 2 níveis no dado de Stress automaticamente (regra de Supressão, veja Parte III), independentemente de sofrer baixas ou não.
...
\begin{itemize}[leftmargin=1.2cm]
  ...
  \item O aumento de Nível de Stress é aplicado individualmente a cada unidade inimiga atingida.
\end{itemize}
...
\begin{nota}
Se o esquadrão terminar o turno mais espalhado do que isso, a unidade perde a comunicação: sobe 1 nível no dado de Stress automaticamente e é obrigada a usar sua próxima ação para se reunir.
\end{nota}
...
\textbf{Se Acertar (Supressão)}: além do dano, a unidade sobe 1D3 níveis no dado de Stress (role 1D6: 1-2 = sobe 1, 3-4 = sobe 2, 5-6 = sobe 3), independentemente de quantos soldados tenham morrido.

\begin{nota}
Este é o mesmo perfil de dano de área usado pela Bazooka contra infantaria (Parte V, seção Bazooka contra Infantaria): uma única rolagem de Acerto seguida de uma única rolagem de Dano, sem passar pela Sequência de Disparo padrão da Parte III.
\end{nota}
...
\begin{nota}
Historiadores estimam que morteiros e artilharia, não fuzis ou metralhadoras, foram responsáveis pela maioria das baixas da Segunda Guerra Mundial: um lembrete de que o Ajuste de Mira paciente costuma valer mais do que o tiro direto.
\end{nota}
...
\midrule
Leve & 2 Homens & Mín. 30cm -- Máx. 60cm & Pode mover e atirar (1º tiro com -1) & 1D3 Acertos, sobe 1 nível de Stress \\
Pesado / Médio & 3 Homens & Mín. 50cm -- Máx. Infinito & Fixo (se mover, não atira) & 1D6 Acertos, sobe 2 níveis de Stress \\
\bottomrule
\end{tabular}
\end{center}
