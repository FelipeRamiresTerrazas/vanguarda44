...
\hypersetup{
  pdftitle={Vanguarda 44: O Wargame -- Cartoes de Referencia Rapida},
...
\item \textbf{Reagrupar} -- não move/atira, 1D6 = níveis de Stress removidos
\item \textbf{Emboscada} -- espera reação do inimigo
\item \textbf{Abaixar} -- defesa máxima, não move/atira
\end{itemize}

\textbf{Teste de Ordem:} 1D6 $\geq$ TN (3+ base, fixo, não escala com Stress). Falha: vai para Abaixar, não ativa de novo neste turno.
...
1+ acerto de arma leve = sobe 1 nível no dado de Stress. Armas de Suporte/HE (LMG/HMG/explosivos) = sobe 2 níveis.
}

\cardcell{105}{149}{4}{CQC \& MORAL (DADO DE STRESS)}{
...
\textbf{Recebendo Stress:} arma leve (rifle/pistola/SMG) com 1+ acerto = sobe 1 nível; LMG/HMG/HE (morteiro, tanque, bazuca) = sobe 2 níveis. Nível máx.\ 6. Veterano ignora o 1º nível que subiria a cada turno.

\textbf{Efeitos por Nível:}
\begin{itemize}
\item Nível 1-2: -5cm de movimento.
\item Nível 3-5: -10cm de movimento, -1 no Acerto, -10cm de alcance.
\item Nível 6: pânico, unidade removida da mesa (exceto doutrina Fallschirmjäger/SS).
\end{itemize}
Sem teste de ativação extra: efeitos acima aplicam direto.
}
...
\begin{itemize}
\item \textbf{Ordem Recuperar (Rally):} custo 1 PC do Sargento (ou Tenente perto); não move/atira; role 1D6 e reduza o Nível de Stress nesse valor (mín.\ 0, remove o dado); sai do Abaixar.
...
\textbf{Granadas} (alcance 15cm, só vs Cobertura Pesada, 1/esquadrão/turno): mesmo perfil da Bazooka vs infantaria, e sempre sobe 1D3 níveis de Stress (1-2=sobe 1, 3-4=sobe 2, 5-6=sobe 3).

\textbf{Morteiros (Ajuste de Mira):} 1º tiro precisa de 6, 2º de 5+, 3º+ de 4+ (reseta para 6 se o alvo se mover).
\begin{itemize}
\item Leve (2H): 1D3 acertos, sobe 1 nível de Stress.
\item Pesado (3H): 1D6 acertos, sobe 2 níveis de Stress.
\end{itemize}
...
\textbf{HMG:} equipe de 2 (Atirador + Municiador), unidade de Suporte independente. Poder de Negação: acerto = sobe 2 níveis no dado de Stress automaticamente. Arco de Fogo 45°; girar além custa +1 PC (Reposicionar). Sem PC de oficial: Autônoma, -1 no Acerto.
...
\textbf{EUA (Fogo Superior e Mobilidade):} Rifles não sofrem -1 ao Avançar. Regular: Rádio grátis para o Tenente + re-rola Ajuste de Mira "1". Airborne: ignora os 2 primeiros níveis de Stress por turno (1 de Veterano + 1 da doutrina).

\textbf{Alemanha (A Máquina de Serrar):} LMG/HMG recebem +1 dado; Municiador morto = -2 no Acerto (em vez de -1). Heer: perda do Sargento não sobe o dado de Stress (promoção automática). Fallschirmjäger/SS: ao chegar no Nível 6, role 1D6 -- com 4+ a unidade permanece em campo (sofrendo as penalidades do Nível 6) em vez de ser removida, até reduzir ao menos 1 nível.
}

\cardcell{105}{297}{10}{ALCANCES \& MODIFICADORES}{
\textbf{Alcance das Armas:}

\begin{tabular}{@{}lc@{}}
\toprule
\textbf{Arma} & \textbf{Alcance}\\
\midrule
SMG & 20cm\\
Rifle & 60cm\\
LMG/BAR & 60--80cm\\
...
\vspace{3pt}
\textbf{Modificadores de Disparo:}

\begin{tabular}{@{}p{3.4cm}p{2.4cm}@{}}
\toprule
\textbf{Situação} & \textbf{Mod.}\\
\midrule
Campo Aberto & 0\\
Cobertura Leve & -1\\
Cobertura Pesada & -2\\
...
\vspace{3pt}
6 natural sempre acerta; 1 natural sempre erra. Modificadores cumulativos entre si.
}
